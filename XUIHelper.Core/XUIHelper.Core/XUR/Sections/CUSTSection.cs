...
{
    public abstract class CUSTSection : ICUSTSection
    {
        ...
        public async Task<bool> TryReadAsync(IXUR xur, BinaryReader reader)
        {
            try
            {
                xur.Logger = xur.Logger?.ForContext(typeof(CUSTSection));
                xur.Logger?.Here().Verbose("Reading CUST section.");
            ...
            {
                xur.Logger?.Here().Error("Caught an exception when reading CUST section, returning false. The exception is: {0}", ex);
                return false;
            }
        }
        ...
        public async Task<bool> TryBuildAsync(IXUR xur, XUObject xuObject)
        {
            ...
            {
                xur.Logger = xur.Logger?.ForContext(typeof(CUSTSection));
                xur.Logger?.Here().Verbose("Building CUST figures.");
                ...
                xur.Logger?.Here().Verbose("Built a total of {0} CUST figures successfully!", Figures.Count);
                return true;
            }
            catch (Exception ex)
            {
                xur.Logger?.Here().Error("Caught an exception when trying to build CUST figures, returning false. The exception is: {0}", ex);
                return false;
            }
        }
        ...
        public async Task<int?> TryWriteAsync(IXUR xur, XUObject xuObject, BinaryWriter writer)
        {
            try
            {
                ...
                xur.Logger?.Here().Verbose("Wrote a total of {0} customs as {1:X8} bytes successfully!", Figures.Count, bytesWritten);
                return bytesWritten;
            }
            catch (Exception ex)
            {
                xur.Logger?.Here().Error("Caught an exception when writing CUST section, returning null. The exception is: {0}", ex);
                return null;
            }
        }
    }
